function StepItem({ number, text }) {
  return (
    <div className="flex gap-3 items-start">
      <span className="flex-shrink-0 w-8 h-8 rounded-full bg-[var(--color-border-light)] flex items-center justify-center text-[14px] font-semibold font-[var(--font-mono)] text-[var(--color-text-muted)]">
        {number}
      </span>
...
export default function StepFlow({ stepFlow }) {
  ...
  if (!hasPhases) {
    return (
      <div className="my-8 bg-[var(--color-bg-white)] border border-[var(--color-border)] rounded-lg p-5 space-y-3">
  ...
  return (
    <div className="my-8 space-y-4">
      {stepFlow.phases.map((phase, pi) => (
        <div key={pi} className="bg-[var(--color-bg-white)] border border-[var(--color-border)] rounded-lg p-5">
